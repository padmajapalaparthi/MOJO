...
exports.getAvailableTables = exports.deleteTable = exports.createTable = exports.getTables = void 0;
...
const createTable = async (req, res) => {
    try {
        ...
        const tableExists = await Table_1.default.findOne({ tableNumber });
        if (tableExists) {
            return res.status(400).json({ message: 'Table number already exists' });
        }
        const table = new Table_1.default({ tableNumber, capacity, isActive });
...
const deleteTable = async (req, res) => {
    try {
        const table = await Table_1.default.findById(req.params.id);
        if (table) {
...
const getAvailableTables = async (req, res) => {
    try {
        ...
        const reservationsOnDate = await Reservation_1.default.find({ date, status: { $ne: 'Cancelled' } });
        ...
        const availableTables = tables.filter((table) => {
            ...
            const hasOverlap = reservationsOnDate.some((resv) => {
                if (resv.table.toString() !== table._id.toString())
                    return false;
